...
class ExpandableBuffer {
    ...
    constructor() {
        ...
        this.initialBufferLength = 4800;  // 200ms at 24kHz - faster start, still smooth
    ...
    logTimeElapsedSinceLastWrite() {
        ...
        if (this.lastWriteTime !== 0) {
            ...
            // Only log if elapsed time is unusually long (> 1 second)
            if (elapsed > 1000) {
    ...
    write(samples) {
        ...
        if (this.writeIndex + samples.length <= this.buffer.length) {
            ...
            if (samples.length <= this.readIndex) {
                ...
                // Shifting buffer (normal operation)
                ...
                // Expanding buffer (normal operation)
    ...
    read(destination) {
        ...
        if (copyLength > 0 && this.underflowedSamples > 0) {
            // Recovered from buffer underflow
            this.underflowedSamples = 0;
        }
    ...
    constructor() {
        ...
        this.port.onmessage = (event) => {
            if (event.data.type === "audio") {
                ...
                // Reset buffering state so the new buffer length takes effect
                this.playbackBuffer.isInitialBuffering = true;
            }
